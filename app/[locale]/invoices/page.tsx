...
const Page: React.FC = () => {
  ...
  const [taxFilter, setTaxFilter] = useState<string>("all-documents");
  ...
  useEffect(() => {
    ...
    if (!showTax) setValue("details.taxDetails.amount", 0);
    if (!showDiscount) setValue("details.discountDetails.amount", 0);
    if (!showShipping) setValue("details.shippingDetails.cost", 0);
  ...
  const onGeneratePdf = async (invoice: Invoice) => {
    ...
    try {
      ...
      setToast({ title: "Success", description: "PDF generated successfully" });
    } catch (error) {
  ...
  const filterInvoices = useCallback(() => {
    return invoices.filter((invoice) => {
      ...
      const matchesDocumentType =
        documentType === "all" ||
        (documentType === "invoices" && invoice.details.isInvoice) ||
        (documentType === "quotations" && !invoice.details.isInvoice);

      // Fixed line — no more type error
      const hasTax = Number(invoice.details.taxDetails?.amount || 0) > 0;
      const matchesTaxFilter =
        taxFilter === "all-documents" ||
        (taxFilter === "with-tax-only" && hasTax);
      ...
      return (
        matchesSearch && matchesDocumentType && matchesTaxFilter && matchesDate
      );
    });
  }, [invoices, searchTerm, documentType, taxFilter, dateRange]);
  ...
  const downloadFilteredPdf = async () => {
    ...
    try {
      ...
      const a = document.createElement("a");
      a.href = url;
      a.download = `SPC_Documents_List_${new Date()
        .toISOString()
        .slice(0, 10)}.pdf`;
      a.click();
      window.URL.revokeObjectURL(url);
            ...
            <div className="flex flex-wrap items-center gap-3">
              ...
              <DropdownMenu>
                ...
                <DropdownMenuContent align="end">
                  <DropdownMenuItem onClick={() => setDocumentType("all")}>
                    All Documents
                  </DropdownMenuItem>
                  <DropdownMenuItem onClick={() => setDocumentType("invoices")}>
                    Invoices Only
                  </DropdownMenuItem>
                  <DropdownMenuItem
                    onClick={() => setDocumentType("quotations")}
                  >
                    Quotations Only
                  </DropdownMenuItem>
                </DropdownMenuContent>
              ...
              <DropdownMenu>
                ...
                <DropdownMenuContent align="end">
                  <DropdownMenuItem
                    onClick={() => setTaxFilter("all-documents")}
                  >
                    All Tax Status
                  </DropdownMenuItem>
                  <DropdownMenuItem
                    onClick={() => setTaxFilter("with-tax-only")}
                  >
                    With Tax Only
                  </DropdownMenuItem>
                </DropdownMenuContent>
