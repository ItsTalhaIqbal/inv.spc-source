    ...
    .map((item: Item, index: number) => {
      ...
      return `
        <tr class="border">
          ...
          <td class="w-[15%] text-center text-black text-base border border-gray-500">${unitPrice || ""}</td>
          <td class="w-[15%] text-center text-black text-base border border-gray-500">${total || ""}</td>
        </tr>
      `;
    })
    .join("");
